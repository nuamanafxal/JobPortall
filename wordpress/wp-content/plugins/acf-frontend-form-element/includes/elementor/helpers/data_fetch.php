...
function acfef_get_field_names( $setting ) {
	$field_names = '';
	preg_match_all( '/\[\$acf:(.+?)?\]/', $setting, $matches );
	array_shift( $matches );
	$field_names = json_encode( $matches[0] );
	return $field_names;
}

function acfef_get_code_value( $setting, $post_id = false , $placeholder = false ) {
	if( ! $post_id ){
		global $post;
		$post_id = $post->ID;
	} 
		
	return preg_replace_callback( '/\[\$(.+?)?\]/', function( $matches ) use( $post_id, $placeholder ) {
		$value = '';
		$return_content = explode( ':', $matches[1] );
		if( $placeholder ){
			$placeholder = ucwords( str_replace('_', ' ', $return_content[1] ) );
			return '[' . $placeholder . ']';
		} 
		if( $return_content[0] == 'user' ){
			$value = acfef_get_user_match_value( $return_content, $post_id );			
		}	
		if( $return_content[0] == 'post' ){
			$value = acfef_get_post_match_value( $return_content, $post_id );
		}	
		
		if( $return_content[0] == 'acf' ){
